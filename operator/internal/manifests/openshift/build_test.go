...
func TestBuild_MonitoringClusterRoleRefMatches(t *testing.T) {
	opts := NewOptions("abc", "ns", "abc", "example.com", "abc", "abc", map[string]string{}, true, false, false, map[string]TenantData{})

	objs := Build(opts)
	cr := objs[5].(*rbacv1.Role)
	rb := objs[6].(*rbacv1.RoleBinding)
